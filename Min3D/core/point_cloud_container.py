...
class PointCloudContainer(GeometryContainer):
    ...
    @classmethod
    @overrides
    def from_ply(cls, file_path: str, **kwargs) -> "PointCloudContainer":
        point_cloud = o3d.io.read_point_cloud(file_path)
        return cls(geometry=point_cloud, **kwargs)
    
    @classmethod
    @overrides
    def from_o3d(cls, geometry: o3d.geometry.PointCloud, **kwargs) -> "PointCloudContainer":
        return cls(geometry=geometry, **kwargs)
    
    @classmethod
    def from_table(cls, table: np.ndarray, **kwargs) -> "PointCloudContainer":
        geometry = o3d.geometry.PointCloud()
        geometry.points = o3d.utility.Vector3dVector(table)
        return cls(geometry=geometry, **kwargs)
    
    @classmethod
    def from_MinSpt(cls, MinSpt_dataset: Any, **kwargs) -> "PointCloudContainer":
        geometry = o3d.geometry.PointCloud()
        geometry.points = o3d.utility.Vector3dVector(MinSpt_dataset.to_table(select_axes = ['X', 'Y', 'Z']))
        return cls(geometry=geometry, **kwargs)
    ...
    # %% Data Manipulation        
    def scale_axis(self, scale_factors: Tuple[float, float, float], center: Union[np.ndarray, bool] = True, inplace: bool = True) -> Union["PointCloudContainer", NoReturn]:
        if isinstance(center, bool) and center:
            self.center_on_origin(inplace=True)
        if isinstance(center, np.ndarray):
            self.translate(-center, inplace=True)
        
        points = self.get_table()
        ...
        if inplace:
            ...
            return PointCloudContainer.from_o3d(new_dataset)
        
    def downsample_pcd_uniformly(self, every_k_point: float, inplace: bool = True) -> Union["PointCloudContainer", NoReturn]:
        downsampled_pcd = self.geometry.uniform_down_sample(input = self.geometry, every_k_points=every_k_point)
        if inplace:
            self.geometry = downsampled_pcd
        return PointCloudContainer.from_o3d(downsampled_pcd)
    
    def statistical_outlier_removal(self, nb_points: int = 20, std_ratio: float = 2.0, inplace:bool = True, display_result: bool = True) -> Union["PointCloudContainer", NoReturn]:
        # call the open3d function for statistical outlier removal
        cl, ind = self.geometry.remove_statistical_outlier(nb_neighbors=nb_points, std_ratio=std_ratio)
        ...
        if inplace:
            ...
            return PointCloudContainer.from_o3d(cl)
        
    def radius_outlier_removal(self, nb_points: int = 16, radius: float = 0.05, inplace:bool = True, display_result: bool = True) -> Union["PointCloudContainer", NoReturn]:
        # call the open3d function for radius outlier removal
        cl, ind = self.geometry.remove_radius_outlier(nb_points=nb_points, radius=radius)
    ...
    # %% Measurement plots
    def plot_spread_of_points_around_center(self, return_fig: bool = False) -> Union[Tuple[plt.Figure, plt.Axes], NoReturn]:
        center_of_mass = self.get_center_of_mass()
        distance_to_center_of_mass = np.linalg.norm(np.asarray(self.geometry.points) - center_of_mass, axis=1)
    ...
    def plot_2d_histogram_of_points(self, return_fig: bool = False) -> Union[Tuple[plt.Figure, plt.Axes], NoReturn]:
        points = np.asarray(self.geometry.points)
        center_of_mass = self.get_center_of_mass()
    ...
    def plot_dNN(self):
        self.plot_dNN_for(self.geometry)
    
    def plot_dNN_for(self, *args:o3d.geometry.PointCloud, x_lim: Tuple[float, float] = None, return_fig: bool = False)->Union[Tuple[plt.Figure, plt.Axes], NoReturn]:
    ...
    def get_kNN(self, point: np.ndarray, k: int) -> Tuple[np.ndarray, np.ndarray]:
        [k, idx, _] = o3d.geometry.KDTreeFlann(self.geometry).search_knn_vector_3d(point, k)
    ...
    def get_radiusNN(self, point: np.ndarray, radius: float) -> Tuple[np.ndarray, np.ndarray]:
        [k, idx, _] = o3d.geometry.KDTreeFlann(self.geometry).search_radius_vector_3d(point, radius)
        return np.asarray(self.geometry.points)[idx], idx
    ...
    # %% Dunder methods
    @overrides
    def __repr__(self) -> "PointCloudContainer":
        return f"PointCloudContainer with {len(self.geometry.points)} points."